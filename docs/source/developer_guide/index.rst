...
``aiida-gmsh`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-gmsh`` repository (preferably using ``aiida-gmsh`` as the project name)

The documentation is now available at `aiida-gmsh.readthedocs.io <http://aiida-gmsh.readthedocs.io/>`_.
...
.. note::
   ...
   When updating the plugin package to a new version, remember to update the version number both in ``setup.json`` and ``aiida_gmsh/__init__.py``.
