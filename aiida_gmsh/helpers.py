...
Helper functions for setting up
 ...
 2. A "gmsh" code on localhost

Note: Point 2 is made possible by the fact that the ``gmsh`` executable is
...
def get_computer(name=LOCALHOST_NAME, workdir=None):
    ...
    try:
        ...
        computer = Computer(
            label=name,
            description='localhost computer set up by aiida_gmsh tests',
            hostname=name,
            workdir=workdir,
